...
def check_similarity(new_embedding, existing_embeddings):
    ...
    # Convert existing embeddings from strings to lists of floats if necessary
    existing_embeddings = [
...
def process_articles():
    ...
    # Get existing articles from embeddings table to avoid duplicates
    existing_response = supabase.table('embeddings').select('content, embedding').execute()
    existing_contents = {record['content'] for record in existing_response.data if record.get('content')}
    ...
    for article in articles:
        ...
        # Skip if this content already exists in the embeddings table
        if formatted_text in existing_contents:
        ...
        try:
            ...
            result = supabase.table('embeddings').insert({
                ...
                'article_id': article['id']  # Add the article_id from the Articles table
            }).execute()

            if result.data:
                processed_count += 1
                print(f"Successfully processed article: {article['title']}")
                # Add to existing collections for future comparisons
                existing_contents.add(formatted_text)
...
def update_embeddings():
    """
    For rows in the 'embeddings' table that do not have an embedding yet,
    generate and update the embedding.
    """
    # Select rows where the embedding column is null.
    ...
    # Retrieve existing embeddings (non-null) for similarity check.
    ...
    for record in records:
        ...
        if embedding is None:
            print(f"Failed to generate embedding for record {record['id']}")
            continue
